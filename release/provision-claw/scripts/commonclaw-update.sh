...
# whose standard error went to a file answers no, and a ride reading that file
# still gets the line.
#
...
# ONE LINE PER EVENT. Under the update timer the logger line is already in the
...
# tag. An operator's ride redirects this stream to a file, JOURNAL_STREAM then
# names no stream this run holds, and the copy is written the way it was.
log() {
  logger -t commonclaw-update -p "user.$1" -- "$2" 2>/dev/null || true
